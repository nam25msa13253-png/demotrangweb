...
-- Ban RUT GON danh rieng cho trinh Query Editor tren web cua Railway (tab Database > Data).
--
-- Vi sao can ban rieng nay: trinh Query Editor cua Railway chay trong 1 database co san
-- (da duoc chon san o thanh ben trai, thuong ten "railway"), va khong xu ly tot cac lenh
-- DROP DATABASE / CREATE DATABASE / USE gop chung trong 1 lan chay (bao loi cu phap gan
-- dong "CREATE DATABASE..."). Ban goc `schema.sql` (dung cho phpMyAdmin/XAMPP local hoac
-- mysql.exe CLI) van giu nguyen 3 dong do - CHI file nay bo di.
--
-- Cach dung: copy toan bo noi dung file nay, dan vao khung Query cua Railway (tab Data),
-- bam Run Query (hoac Ctrl+Enter). Tat ca bang se duoc tao ngay trong database mac dinh
-- Railway da cap san - nen khi dien DB_NAME vao Render, dung ten database do (vd "railway"),
-- KHONG dung "smart_queue" vi file nay khong tao database moi.
-- =====================================================================================
...
-- ---------------------------------------------------------------------------
-- audit_logs: Nhat Ky Kiem Toan - append-only
-- ---------------------------------------------------------------------------
